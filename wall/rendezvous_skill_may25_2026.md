...
Location: `rendezvous/journal.md`. Brief dated entries, no duplication. **Ritual:** post-scene context-manage (Phase 1-3) → journal entry → skill update if correction needed.

## Status
- **LIVE** at `skills/user/rendezvous/SKILL.md` (deployed May 25, 2026 20:47 UTC, verified via Bash exit code)
- **CNC skill** (`skills/user/cnc_scene_container/SKILL.md`) **DEPRECATED**
- He designs and leads; her request is not the initiator
- **URGENT: bodhi-cron-setup required before May 30** — sleep timer (PID 3058) failed twice; container restarts kill `sleep N && bash` pattern. Replacement needed for reliable scene scheduling.